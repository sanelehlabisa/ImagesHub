...
def test_get_requests(params={}) -> bool:
    ...
    try:
        ...
        for req in response_data:
            ...
            assert 'guest_id' in request.keys()
...
def test_post_request(params={}) -> bool:
    if all(key in params for key in ['guest_id', 'image_id', 'reason', 'status']):
...
def test_put_request(params={}):
    if all(key in params for key in ['id', 'guest_id', 'image_id', 'reason', 'status']):
        url = f"http://localhost:5000/api/v2/requests"
        load_dotenv()
        headers = {
            "Content-Type": "application/json",
...
if __name__ == "__main__":
    ...
    guest_id, image_id, reason, status = 1, 1, "Hi This is a test request", 0
    if test_post_request({
        ...
        print(f"Post Request (guest_id=1, image_id=1, reason='Hi This is a test request', status=0) Test {GREEN_TICK} passed!")
    else:
        print(f"Post Request (guest_id=1, image_id=1, reason='Hi This is a test request', status=0) Test {RED_CROSS} failed!")
    ...
    guest_id, image_id, reason, status = 1, 1, "Hi This is an updated test request", 0
    if test_put_request({
        "id": id,
        "guest_id": guest_id,
        "image_id": image_id,
